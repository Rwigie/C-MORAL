...
class GRPOTrainer:
    ...
    def collect_rollouts(self):
        ...
        stats = {
            ...
            "mean_scores": {k: v.mean().item() for k, v in self.buffer.scores.items()},
    ...
    def evaluate(self, iteration: int, verbose: bool = False, extended: bool = False):
        ...
        metrics = {"reward_mean": eval_reward_mean, **compute_property_mean(result, self.config.task), **compute_score_mean(result, self.config.task)}
        if verbose:
            print(
                "[Eval]\n"
