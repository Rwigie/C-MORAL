...
from .rollout import GRPORolloutBuffer
...
class GDPOTrainer:
    ...
    def __init__(
        ...
        self.device = device
        if config is None:
            raise ValueError("config must be provided to GRPOTrainer.")
        self.config = config
    ...
    def _init_schedulers(self):
        ...
        init_lr = float(getattr(cfg, "lr_actor", 0.0) or 0.0)
        min_lr = float(getattr(cfg, "min_lr_actor", 0.0) or 0.0)
        ...
        self.actor_scheduler = torch.optim.lr_scheduler.LambdaLR(self.optimizer_actor, lr_lambda=lr_lambda)
    ...
    def _compute_geometric_reward_from_scores(self, scores_dict) -> float:
        """
        Compute mean geometric score from sigmoid-transformed per-property scores.
        This is used only for logging/model selection, not for advantage aggregation.
        """
        if not isinstance(scores_dict, dict) or not scores_dict:
            return 0.0

        reward_cfg = self.config.rl_task_cfg or {}
        task_name = reward_cfg.get("task", self.config.task) if isinstance(reward_cfg, dict) else self.config.task
        use_similarity = reward_cfg.get("use_similarity", True) if isinstance(reward_cfg, dict) else True
        task_keys = [k.strip().lower() for k in (task_name or "").split("+") if k.strip()]
        keys = list(dict.fromkeys(task_keys + (["sim"] if use_similarity else [])))
        ...
        score_stack = torch.stack(tensors, dim=1).clamp(min=1e-6, max=1.0)
        # Geo Reward = mean over per-sample geometric means.
        sample_geo = torch.exp(torch.log(score_stack).mean(dim=1))
        return float(sample_geo.mean().item())
    ...
    def train(self):
        ...
        try:
            for iteration in range(self.config.num_iterations):
                ...
                for (k, v) in self.buffer.scores.items():
                    print(f" Score {k}: mean={v.mean().item():.4f}")
                ...
                train_metrics = {
                    ...
                    "geo_reward_mean": rollout_stats["geo_reward_mean"],
                ...
                train_metrics.update({f"mean_scores/{k}": v for k, v in rollout_stats["mean_scores"].items()})
                train_metrics.update({f"mean_values/{k}": v for k, v in rollout_stats["mean_values"].items()})
                ...
                print(
                    ...
                    f"  Mean Reward  : {rollout_stats['mean_reward']:.4f}\n"
                    f"  Geo Reward   : {rollout_stats['geo_reward_mean']:.4f}\n"
                    f"  KL Loss      : {metrics['kl_loss_mean']:.4f}\n"
                    f"  Mean Scores  : {', '.join([f'{k}: {v:.4f}' for k, v in rollout_stats['mean_scores'].items()])}\n"
                    f"  Mean Values  : {', '.join([f'{k}: {v:.4f}' for k, v in rollout_stats['mean_values'].items()])}"
                )

                if iteration % self.config.eval_interval == 0 and iteration != 0:
                    if iteration % (5 * self.config.eval_interval) == 0:
                        self.evaluate(iteration, verbose=True, extended=False)
                    else: 
                        self.evaluate(iteration, verbose=True, extended=False)
    ...
    def collect_rollouts(self):
        ...
        mean_scores = {k: v.mean().item() for k, v in scores.items()}
        geo_reward_mean = self._compute_geometric_reward_from_scores(scores)
        reward_scale = float((self.config.rl_task_cfg or {}).get("reward_scale", 1.0))
        mean_reward = geo_reward_mean * reward_scale
        mean_values = compute_property_mean(
            {
                "target_properties": self.buffer.target_properties,
        ...
        stats = {
            ...
            "geo_reward_mean": geo_reward_mean,
    ...
    def _run_ppo_epochs(self):
        ...
        kl_losses = []
        reward_cfg = self.config.rl_task_cfg or {}
        # advantages = compute_grpo_advantages_per_props(
        #     num_envs = self.config.num_envs,
        #     num_return_sequences = self.config.num_return_sequences,
        #     scores_dict = self.buffer.scores,
        #     weights_dict = reward_cfg.get("reward_weight"),
        #     validity_mask = self.buffer.masking_mask,
        #     labels = self.buffer.labels,
        #     old_log_probs = self.buffer.log_probs,
        #     ref_log_probs = self.buffer.ref_log_probs,
        #     kl_coef = self.adaptive_kl.kl_coef,
        #     task = reward_cfg.get("task", self.config.task),
        ...
        for epoch in tqdm(range(cfg.ppo_epochs), desc="PPO Epochs"):
            ...
            if avg_kl > self.config.early_stop_kl:
                ...
                break           
            self.adaptive_kl.update(avg_kl)
    ...
    def _save_checkpoint(self, iteration: int):
        target_dir = os.path.join(self.run_dirs["checkpoints_dir"], f"iter_{iteration}")
        os.makedirs(target_dir, exist_ok=True)
        self.actor.save_lora(target_dir)
        torch.save(self.optimizer_actor.state_dict(), os.path.join(target_dir, "optimizer_actor.pt"))
    ...
    def update(
        ...
        mb_kl_value = mb_kl.item()
        del policy_loss, mb_kl

        return policy_loss_value, mb_kl_value

    # ------------------------------------------------------------- evaluation #
    @torch.no_grad()
    def evaluate(self, iteration: int, verbose: bool = False, extended: bool = False):
        
        if extended:
            eval_len = self.eval_env.size
            print(f"[Eval] Running extended evaluation over {eval_len} samples...")
        else:
            eval_len = self.config.eval_num_envs
        
        chunk_size = 128
        eval_num_envs = int(eval_len)
        if eval_num_envs <= chunk_size:
            ...
            for start in range(0, eval_num_envs, chunk_size):
                ...
                chunk_scores = chunk_result.get("scores", {})
                if isinstance(chunk_scores, dict):
                    for k, v in chunk_scores.items():
                        if isinstance(v, torch.Tensor):
                            scores_chunks.setdefault(k, []).append(v.detach().cpu())
                print(f"[Eval] Evaluated {end} / {eval_num_envs} samples...")
            result = {
                "rewards": torch.cat(reward_chunks, dim=0) if reward_chunks else torch.tensor([]),
                "target_properties": target_properties,
                "scores": {
                    k: torch.cat(v, dim=0) if v else torch.tensor([])
                    for k, v in scores_chunks.items()
                },
            }
        ...
        task_keys = [k.strip().lower() for k in (task_name or "").split("+") if k.strip()]
        keys = list(dict.fromkeys(task_keys + (["sim"] if use_similarity else [])))
        score_means = compute_score_mean(result, self.config.task)
        geo_reward_mean = self._compute_geometric_reward_from_scores(result.get("scores", {}))
        reward_scale = float((self.config.rl_task_cfg or {}).get("reward_scale", 1.0))
        eval_reward = geo_reward_mean * reward_scale
        metrics = {
            "reward_mean": eval_reward,
            "geo_reward_mean": geo_reward_mean,
            **compute_property_mean(result, self.config.task),
            **score_means,
        }
        if verbose:
            print(
                "[Eval]\n"
                f" Mean Reward: {eval_reward:.4f}\n"
                f" Geo Reward : {geo_reward_mean:.4f}\n"
                f" Mean Scores: {', '.join([f'{k}: {v:.4f}' for k, v in score_means.items()])}"
            )
            for k in keys:
                v = metrics.get(f'{k}_value')
                if v is None or not isinstance(v, (int, float)) or not math.isfinite(v):
                    print(f" {f'{k}_value'}: nan")
                else:
                    print(f" {f'{k}_value'}: {v:.4f}")
                    
            for k in keys:
                v = metrics.get(f"{k}_score")
                if v is None or not isinstance(v, (int, float)) or not math.isfinite(v):
                    print(f" {f'{k}_score'}: nan")
                else:
                    print(f" {f'{k}_score'}: {v:.4f}")
        ...
        if eval_reward > self.best_reward:
            ...
            if hasattr(self.actor, "save_lora"):
                self.actor.save_lora(self.run_dirs["best_adapter_dir"])
            elif hasattr(self.actor, "model") and hasattr(self.actor.model, "save_pretrained"):
                save_best_adapter(self.actor.model, self.run_dirs["best_adapter_dir"])
        return eval_reward
